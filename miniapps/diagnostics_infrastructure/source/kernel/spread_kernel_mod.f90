...
module spread_kernel_mod
    ...
    use diagnostics_infra_config_mod, only: blending_percentage
...
contains
    ...
    ! spread_kernel_code: The subroutine which is called directly by the Psy layer
    ! number_of_layers: the number of layers
    ! field: target field to be worked on
    ! field_variance: the outcome change to apply to the field later on - note because of the stencil the field is read only
    ! stencil_size:
    ! stencil_map:
    ! degrees_of_freedom: degrees of freededom
    ! unique_degrees_of_freedom: total number of degrees of freedom across the column
    ! field_dof_map: dof map for bottom cell of field
    ! blend_percentage: percentage of difference to add to the target. normally 1-100. default 100. optional.
