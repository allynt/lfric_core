...
!> @brief Contains the quadrature_3d_xyz_type that accepts a quadrature rule

!> @details This module contains the quadrature_3d_xyz_type where the weights
!> and points are stored in 3D (x-y-z horizontal & vertical). To construct this 
!> type a quadrature rule (quadrature_rule_type) is passed. Currently there are 
!> two supported rules: Gaussian and Newton-Cotes.

module quadrature_3d_xyz_mod
...
! Public types
!-------------------------------------------------------------------------------
type, public, extends(quadrature_abstract_type) :: quadrature_3d_xyz_type
  private
  !> allocatable arrays which holds the quadrature weights (w) and points (x)
  real(kind=r_def), allocatable :: xqp_xyz(:,:), wqp_xyz(:)

  !> Number of quadrature points xy direction = nqp_x*nqp_y*nqp_z
...
contains
  ...
  !> quadrature_3d_xyz type.
  procedure, public :: get_proxy

  !> @brief Routine to destroy quadrature
  final     :: quadrature_destructor

end type quadrature_3d_xyz_type

!> Psy layer representation of a quadrature_3d_xyz type
!>
!> This is an accessor class that allows access to quadrature_3d_xyz_type 
!> data and information
!> with each element accessed via a public pointer.
!>
type, public :: quadrature_3d_xyz_proxy_type

  private
  !> allocatable arrays which holds the values of the gaussian quadrature
  real(kind=r_def), pointer, public :: xqp_xyz(:,:) => null()
  real(kind=r_def), pointer, public :: wqp_xyz(:)   => null()

  !> Number of quadrature points xy direction = nqp_x*nqp_y*nqp_z
  integer, pointer, public          :: nqp_xyz      => null()
...
contains
end type quadrature_3d_xyz_proxy_type

!-------------------------------------------------------------------------------
! Module parameters
!-------------------------------------------------------------------------------

interface quadrature_3d_xyz_type
...
!>        quadrature_3d_xyz_type.
!>
!> @return The proxy type with public pointers to the elements of
!> quadrature_3d_xyz_type.
type(quadrature_3d_xyz_proxy_type ) function get_proxy(self)

  implicit none

  class(quadrature_3d_xyz_type), target, intent(in)  :: self

  get_proxy % xqp_xyz              => self % xqp_xyz
  get_proxy % wqp_xyz              => self % wqp_xyz
  get_proxy % nqp_x                => self % nqp_x
  get_proxy % nqp_y                => self % nqp_y
  get_proxy % nqp_z                => self % nqp_z
  get_proxy % nqp_xyz              => self % nqp_xyz
...
function init_quadrature_uniform(nqp, rule) result (self)
  ...
  type(quadrature_3d_xyz_type) :: self
...
!-------------------------------------------------------------------------------
!> @brief Initialises the quadrature rule with explicit size in each dimension.
...
function init_quadrature_varible(nqp_x, nqp_y, nqp_z, rule) result (self)
  ...
  self%nqp_xyz = nqp_x*nqp_y*nqp_z
...
subroutine create_quadrature(self, rule)
  ...
  class(quadrature_3d_xyz_type) :: self
  class(quadrature_rule_type), intent(in) :: rule
  ...
  allocate( self%xqp_xyz(3,self%nqp_xyz) )
  allocate( self%wqp_xyz(self%nqp_xyz) )

  ! Initilise all to zero
  self%xqp_xyz(:,:) = 0.0_r_def
  self%wqp_xyz(:) = 0.0_r_def
  ...
  do i=1,self%nqp_x
    do j=1,self%nqp_y
      do k=1,self%nqp_z
        self%xqp_xyz(1,ic) = points_weights_x(i,1)
        self%xqp_xyz(2,ic) = points_weights_y(j,1)
        self%xqp_xyz(3,ic) = points_weights_z(k,1)

        self%wqp_xyz(ic) = points_weights_x(i,2)*points_weights_y(j,2)*points_weights_z(k,2)
...
subroutine quadrature_destructor(self)
  ...
  if(allocated(self%xqp_xyz)) deallocate(self%xqp_xyz)
  if(allocated(self%wqp_xyz)) deallocate(self%wqp_xyz)
  
end subroutine quadrature_destructor

end module quadrature_3d_xyz_mod
