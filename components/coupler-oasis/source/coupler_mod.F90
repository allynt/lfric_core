...
!-----------------------------------------------------------------------------
!> coupling related routines for use in coupled configuration
...
#if defined(MCT) && defined(UM_PHYSICS)
...
!can handle only cases when coupling frequency is the same for all components
...
!lon and lat for faces
    local_mesh =>  mesh_collection%get_mesh( mesh_id )
    num_levels = local_mesh%get_nlayers()
...
!initialize scratch variables
     call field1%initialise( vector_space = fld_cpld_fs, name="cpl_field1_W3" )
     call field2%initialise( vector_space = fld_cpld_fs, name="cpl_field2_W3" )
     call field3%initialise( vector_space = fld_cpld_fs, name="cpl_field3_W3" )
