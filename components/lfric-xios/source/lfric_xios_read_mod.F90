...
subroutine checkpoint_read_value(io_value, value_name)
  ...
  select type(concrete_io_value => io_value)

    type is (int32_arr_io_value_type)
      array_dims = size(concrete_io_value%value)
      allocate(dp_equiv(array_dims))
      call xios_recv_field( restart_id, dp_equiv(1:array_dims) )
      concrete_io_value%value = int(dp_equiv, int32)
      deallocate(dp_equiv)
    ...
    type is (int64_arr_io_value_type)
      ...
      allocate(dp_equiv(array_dims))
      call xios_recv_field( restart_id, dp_equiv(1:array_dims) )
      concrete_io_value%value = int(dp_equiv, int64)
      deallocate(dp_equiv)
    ...
    type is (real32_arr_io_value_type)
      ...
      allocate(dp_equiv(array_dims))
      call xios_recv_field( restart_id, dp_equiv(1:array_dims) )
      concrete_io_value%value = real(dp_equiv, real32)
      deallocate(dp_equiv)
    ...
    type is (real64_arr_io_value_type)
      ...
      allocate(dp_equiv(array_dims))
      call xios_recv_field( restart_id, dp_equiv(1:array_dims) )
      concrete_io_value%value = real(dp_equiv, real64)
      deallocate(dp_equiv)

    class default
      call log_event( "Unsupported type for checkpoint read value: " //  restart_id, &
                      LOG_LEVEL_ERROR )
